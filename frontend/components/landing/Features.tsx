...
function EmailCard({ label, className }: { label: string; className: string }) {
  return (
    <div
      className={`absolute w-32 rounded-md border border-border bg-surface px-2.5 py-2 shadow-sm ${className}`}
    >
...
function ConnectVisual() {
  return (
    <div className="relative h-56 w-full">
      ...
      <Image src="/icon-notes.png" alt="Notes" width={48} height={48} className="absolute left-6 top-4 object-contain drop-shadow-lg" />
      <Image src="/icon-google.png" alt="Google" width={56} height={56} className="absolute left-1/2 top-0 -translate-x-1/2 object-contain drop-shadow-lg" />
      <Image src="/icon-slack.png" alt="Slack" width={48} height={48} className="absolute left-4 bottom-16 object-contain drop-shadow-lg" />
      <Image src="/icon-gmail.png" alt="Gmail" width={48} height={48} className="absolute right-6 top-6 object-contain drop-shadow-lg" />
  ...
  return (
    <div className="h-56 w-full text-[9px]">
      ...
      <div className="mt-1 space-y-2">
        {tasks.map((t) => (
          <div key={t.label}>
            <div className="mb-1 inline-block rounded bg-foreground px-1.5 py-0.5 text-[9px] font-medium text-background">
              {t.label}
            </div>
            {t.rows.map((r, i) => (
    ...
    <section id="features" className="mx-auto max-w-7xl px-4 py-20 md:px-6 md:py-28">
      <div className="mx-auto max-w-2xl text-center">
        ...
        <p className="mt-4 text-sm text-muted-foreground">
          Built with cutting-edge technology to help teams automate, analyze and
          optimize every aspect of their workflow.
        </p>
      </div>
