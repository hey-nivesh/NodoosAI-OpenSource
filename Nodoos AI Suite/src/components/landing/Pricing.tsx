...
const plans = [
  {
    ...
    cta: "Get Started",
    featured: false,
    features: ["Up to 5 AI Agents", "Basic Telemetry Ingest", "Standard Support", "Weekly Reports"],
  },
  {
    name: "Professional",
    price: 49,
    ...
    features: [
      "Unlimited AI Agents",
      "Real-time Snowflake Ingest",
      "CoCo CLI Direct Integration",
  ...
  },
  {
    ...
    cta: "Contact Sales",
    featured: false,
    features: ["Dedicated Cluster", "Custom LLM Fine-tuning", "SLA & SOC2 Compliance", "Dedicated Account Mgr"],
  },
];
    ...
    <section id="pricing" className="mx-auto max-w-7xl px-4 py-20 md:px-6 md:py-28">
      <div className="mx-auto max-w-2xl text-center">
        ...
        <p className="mt-4 text-muted-foreground">
          Flexible plans designed to scale with your data volume and operational velocity.
        </p>
        <div className="mt-6 inline-flex items-center gap-1 rounded-full border border-border bg-surface p-1">
          <button
            onClick={() => setYearly(false)}
            className={`rounded-full px-4 py-1.5 text-sm transition-colors ${!yearly ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
          >
            Monthly
          </button>
          <button
            ...
            className={`rounded-full px-4 py-1.5 text-sm transition-colors ${yearly ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
          >
            Yearly <span className="ml-1 text-[10px] text-accent">-20%</span>
          </button>
        </div>
      </div>
      <div className="mt-14 grid grid-cols-1 gap-5 md:grid-cols-3">
        {plans.map((p, i) => (
          ...
          >
            ...
            <div className={`mt-2 text-lg font-semibold ${p.featured ? "text-primary-foreground" : "text-foreground"}`}>
              {p.name}
            </div>
            ...
            <ul className="mt-6 flex-1 space-y-3">
              {p.features.map((f) => (
                <li key={f} className="flex items-center gap-2 text-sm">
                  <Check className={`h-4 w-4 ${p.featured ? "text-accent" : "text-accent"}`} />
                  <span className={p.featured ? "text-primary-foreground/90" : "text-foreground"}>{f}</span>
                </li>
              ))}
            </ul>
            <button
              className={`mt-8 rounded-full px-6 py-3 text-sm font-medium transition-transform hover:scale-[1.02] ${
                p.featured
